...
export default function HeroSection() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      const words = headingRef.current
        ? gsap.utils.toArray<HTMLElement>(headingRef.current.querySelectorAll(".word"))
        : [];
      ...
      if (circles.length) {
        tl.fromTo(
          ...
          { y: 0, scale: 1, opacity: 0.45, filter: "blur(0px)", stagger: 0.06, duration: 0.85, ease: "power2.out" },
          "-=0.7"
        );
      }

    
      const whatsappLabel = headingRef.current?.querySelector(".whatsapp-label");
      if (whatsappLabel) {
        tl.fromTo(whatsappLabel, { scale: 0.86, opacity: 0 }, { scale: 1, opacity: 1, duration: 0.36 }, "-=0.55");
      }
    }, containerRef);
  ...
  return (
    ...
    >
      ...
      <div style={{ fontFamily: "DavidLibre" }} className="relative z-10 w-full max-w-4xl mx-auto text-center pt-24 sm:pt-28 md:pt-32 lg:pt-32">
        ...
        <h1 ref={headingRef} className="text-2xl sm:text-3xl md:text-5xl max-w-2xl mx-auto ">
          <span>
            <Highlighter action="highlight" color="#FFE5C0">
              <span className="text-orange-600">Your Profile</span>
            </Highlighter>{" "}
          </span>{" "}
          <span className="ml-1">pitched to</span> <span className="text-orange-600">1000's</span> <span>of</span>
          <br />
          <span className="text-xl lg:text-[42px]">Founders, HRs & Paying Clients</span>
      ...
      <div ref={marqRef} className="absolute top-[60%] lg:bottom-0 left-0 w-full flex flex-col justify-center items-center h-[3cm]">
